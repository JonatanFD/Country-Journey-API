...
def filterCities():
    ...
    filtered_cities = [
        city for city in cities if city['country'].lower() in lower_filters]
    set_cityids = {f"{city['city']},{city['country']}".lower()
                   for city in filtered_cities}

    with open(ROUTES_JSON_FILE, 'r') as file:
        routes = json.load(file)

    filtered_routes = [route for route in routes if route['from'].lower(
    ) in set_cityids or route['to'].lower() in set_cityids]

...
def mercator_projection(lat, lon):
    ...
    y = height / 2 - math.log(math.tan(math.pi / 4 + (lat * math.pi) / 180 / 2)
                              ) * (width / (2 * math.pi)) * scaleFactor - height
...
def mercator():
    ...
    south_american_countries = ['AR', 'BO', 'BR', 'CL',
    ...
    filtered_df['longitude'], filtered_df['latitude'] = zip(
        *filtered_df.apply(lambda row: mercator_projection(row['lat'], row['lng']), axis=1))
    json_data = filtered_df[['city', 'country', 'longitude', 'latitude']].to_json(
